...
class TestAudioUtils:
    ...
    def test_init(self, mock_groq):
        with patch.dict(os.environ, {"GROQ_API_KEY": "test_key", "WHISPER_MODEL": "test-model"}):
    ...
    def test_convert_video_to_audio(self, mock_file, mock_tempfile, mock_subprocess, audio_utils, mock_video_bytes):
        ...
        with patch.object(audio_utils.helper_utils, 'create_temp_file', return_value="/tmp/test.mp4"), \
             patch.object(audio_utils.helper_utils, 'cleanup_temp_file'), \
             patch('utils.audio.audio_utils.os.path.exists', return_value=True), \
             patch('utils.audio.audio_utils.os.unlink'):
    ...
    @patch('utils.audio.audio_utils.subprocess.run')
    @patch('utils.audio.audio_utils.tempfile.NamedTemporaryFile')
    def test_convert_video_to_audio_error(self, mock_tempfile, mock_subprocess, audio_utils, mock_video_bytes):
        mock_temp_audio = Mock()
        mock_temp_audio.name = "/tmp/test.wav"
        mock_tempfile.return_value.__enter__.return_value = mock_temp_audio
        ...
        with patch.object(audio_utils.helper_utils, 'create_temp_file'), \
             patch.object(audio_utils.helper_utils, 'cleanup_temp_file'), \
             patch('utils.audio.audio_utils.os.path.exists', return_value=True), \
             patch('utils.audio.audio_utils.os.unlink'):
    ...
    def test_get_transcription(self, audio_utils, mock_video_bytes):
        ...
        with patch.object(audio_utils, '_convert_video_to_audio', return_value=b"audio"), \
             patch.object(audio_utils.client.audio.transcriptions, 'create', return_value=mock_transcription):
    ...
    def test_get_transcription_error(self, audio_utils, mock_video_bytes):
        with patch.object(audio_utils, '_convert_video_to_audio', side_effect=Exception("Error")), \
