...
def sample_global_context():
    ...
    return {"mood": "energetic", "theme": "action", "instruments": ["guitar", "drums"]}
...
class TestLLMUtils:
    ...
    def test_init_missing_api_key(self):
        ...
        with pytest.raises(
            ValueError, match="GROQ_API_KEY environment variable is required"
        ):
            LLMUtils()
    ...
    def test_get_global_config_success(
        self,
        mock_prompts,
        mock_validators,
        mock_groq,
        ...
        mock_validators.make_api_call_with_retry.return_value = (
            mock_groq_client.chat.completions.create.return_value
        )
    ...
    @patch("utils.llm.llm_utils.Groq")
    @patch("utils.llm.llm_utils.LLMValidators")
    def test_get_global_config_error_fallback(
        self, mock_validators, mock_groq, sample_transcript, sample_frames
    ):
    ...
    @patch("utils.llm.llm_utils.LLMValidators")
    @patch("utils.llm.llm_utils.Prompts")
    def test_get_realtime_config_success(
        self,
        mock_prompts,
        mock_validators,
        mock_groq,
        ...
        mock_validators.parse_realtime_llm_response.return_value = (
            mock_realtime_response
        )
        mock_prompts.get_realtime_context_prompt.return_value = {
            "role": "system",
    ...
    @patch("utils.llm.llm_utils.Groq")
    @patch("utils.llm.llm_utils.LLMValidators")
    def test_get_realtime_config_with_invalid_global_context(
        self, mock_validators, mock_groq, sample_transcript, sample_frames
    ):
    ...
    @patch("utils.llm.llm_utils.LLMValidators")
    @patch("utils.llm.llm_utils.Prompts")
    def test_get_realtime_config_with_too_many_frames(
        self,
        mock_prompts,
        mock_validators,
        mock_groq,
        ...
        many_frames = [
            Frame(data=f"base64_image_{i}", timestamp=float(i)) for i in range(10)
        ]
    ...
    @patch("utils.llm.llm_utils.Groq")
    @patch("utils.llm.llm_utils.LLMValidators")
    def test_get_realtime_config_error_fallback(
        self, mock_validators, mock_groq, sample_transcript, sample_frames
    ):
    ...
    @patch.dict(
        "os.environ",
        {"GROQ_API_KEY": "test_api_key", "GROQ_VISION_MODEL": "custom-model"},
    )
    @patch("utils.llm.llm_utils.Groq")
