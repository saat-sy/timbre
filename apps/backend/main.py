...
async def music_websocket_endpoint(websocket: WebSocket):
    ...
    try:
        ...
        required_fields = [Commands.PROMPT, Commands.BPM, Commands.SCALE, Commands.WEIGHT]
        missing_fields = [field for field in required_fields if not data.get(field)]
        
        if missing_fields:
            logger.error("Missing required parameters: %s", ", ".join(missing_fields))
            await websocket.close(code=1003, reason=f"Missing parameters: {', '.join(missing_fields)}")
            return
        ...
        try:
            ...
            if bpm < 30 or bpm > 300:
                ...
                await websocket.close(code=1003, reason="BPM must be between 30 and 300")
                return
            
            if weight < 0.0 or weight > 10.0:
                logger.error("Weight value %.2f is outside valid range (0.0-10.0)", weight)
                await websocket.close(code=1003, reason="Weight must be between 0.0 and 10.0")
                return
